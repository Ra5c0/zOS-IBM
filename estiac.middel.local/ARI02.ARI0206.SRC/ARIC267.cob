      ...
      *---------------------------------------------------------------*
      * 16/04/2025    §  CREATION DU FICHIER ET DEBUT CODE            *
      *               §                                               *
      *===============================================================*
      *
      *************************
      ...
      *
       1020-AIDE-DEB.
           ...
           PERFORM 7070-MOVE-ECRAN7-TO-MAP-DEB
              THRU 7070-MOVE-ECRAN7-TO-MAP-FIN.
      ...
      *
      *---------------------------------------------------------------*
      *               DESCRIPTION DU COMPOSANT CONFIRMATION           *
      *---------------------------------------------------------------*
      *
       1030-CONFIRMATION-DEB.
           ...
           EVALUATE EIBAID
              WHEN DFHENTER
                 PERFORM 2050-ENTER-SUP-DEB
                    THRU 2050-ENTER-SUP-FIN
              WHEN DFHPF3
                 PERFORM 2060-F3-SUP-DEB
                    THRU 2060-F3-SUP-FIN
              WHEN DFHCLEAR
                 PERFORM 2070-ALT-C-SUP-DEB
                    THRU 2070-ALT-C-SUP-FIN
              WHEN OTHER
                 PERFORM 2080-AUTRE-SUP-DEB
                    THRU 2080-AUTRE-SUP-FIN
           END-EVALUATE.
      ...
      *
       2050-ENTER-SUP-DEB.
           ...
           PERFORM 6040-DELETE-ARTICLE-DEB
              THRU 6040-DELETE-ARTICLE-FIN.


           IF WS-RC = DFHRESP(NOTFND)
              PERFORM 7250-MOVE-MSG-DEJA-SUP-DEB
                 THRU 7250-MOVE-MSG-DEJA-SUP-FIN
           ELSE
              PERFORM 7180-MOVE-MSG-ENR-SUP-DEB
                 THRU 7180-MOVE-MSG-ENR-SUP-FIN
           END-IF.
      ...
      *
      *---------------------------------------------------------------*
      *               DESCRIPTION DU COMPOSANT ANNULATION             *
      *---------------------------------------------------------------*
      *
       2060-F3-SUP-DEB.
           ...
           PERFORM 7190-MOVE-MSG-SUP-ABD-DEB
              THRU 7190-MOVE-MSG-SUP-ABD-FIN.
      ...
      *
       2070-ALT-C-SUP-DEB.
           ...
           PERFORM 7030-GEST-PROT-DEB
              THRU 7030-GEST-PROT-FIN

           PERFORM 7230-GEST-ALTC-SUP-DEB
              THRU 7230-GEST-ALTC-SUP-FIN.
      ...
      *
       2080-AUTRE-SUP-DEB.
           ...
           PERFORM 7240-GEST-AUTRE-SUP-DEB
              THRU 7240-GEST-AUTRE-SUP-FIN.
      ...
      *
       4000-CODE-EXISTANT-DEB.
           ...
           PERFORM 7200-MOVE-C-TO-TAFF-DEB
              THRU 7200-MOVE-C-TO-TAFF-FIN.

           PERFORM 7210-MOVE-MSG-CONFIRMATION-DEB
              THRU 7210-MOVE-MSG-CONFIRMATION-FIN.
       ...
       6010-RECEIVE-DEB.
           EXEC CICS
              RECEIVE MAP         (WS-MAP)
                      ...
                      RESP        (WS-RC)
           END-EXEC.
           IF NOT WS-RC = DFHRESP(NORMAL) AND
              NOT WS-RC = DFHRESP(MAPFAIL)
              MOVE WS-RC                    TO WS-RC-ED
      ...
      *---------------------------------------------------------------*
      *
       6040-DELETE-ARTICLE-DEB.
           MOVE WS-COMMAREA-CODE            TO WS-ART-CODE.
           EXEC CICS
              DELETE FILE         ('ART0206')
                     RIDFLD       (WS-ART-CODE)
                     RESP         (WS-RC)
           ...
           IF WS-RC NOT = DFHRESP(NORMAL) AND
              ...
              STRING 'ERREUR DELETE : ' WS-RC-ED
                     DELIMITED BY SIZE
                INTO WS-MSG-FIN
              PERFORM 9999-ABEND-PRG-DEB
                 THRU 9999-ABEND-PRG-FIN
           END-IF.
       6040-DELETE-ARTICLE-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
      ...
      *---------------------------------------------------------------*
      *
       7070-MOVE-ECRAN7-TO-MAP-DEB.
              MOVE 'ARIM267'                TO WS-MAP.
       7070-MOVE-ECRAN7-TO-MAP-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
       7080-MOVE-ECRANHP-TO-MAP-DEB.
           MOVE 'ARIMHP7'                   TO WS-MAP.
       7080-MOVE-ECRANHP-TO-MAP-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
       7090-GEST-UNPROT-DEB.
           MOVE DFHBMPRO                    TO MCODEA.
       7090-GEST-UNPROT-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
      ...
      *---------------------------------------------------------------*
      *
       7180-MOVE-MSG-ENR-SUP-DEB.
           MOVE WS-MSG(30)                  TO MMSGO.
       7180-MOVE-MSG-ENR-SUP-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
       7190-MOVE-MSG-SUP-ABD-DEB.
           MOVE WS-MSG(31)                  TO MMSGO.
       7190-MOVE-MSG-SUP-ABD-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
       7200-MOVE-C-TO-TAFF-DEB.
           MOVE 'C'                         TO WS-TAFF.
       7200-MOVE-C-TO-TAFF-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
       7210-MOVE-MSG-CONFIRMATION-DEB.
           MOVE WS-MSG(29)                  TO MMSGO.
       7210-MOVE-MSG-CONFIRMATION-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
       7220-MOVE-MSG-SUP-INEXIS-DEB.
           MOVE WS-MSG(28)                  TO MMSGO.
       7220-MOVE-MSG-SUP-INEXIS-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
       7230-GEST-ALTC-SUP-DEB.
           MOVE WS-COMMAREA-CODE            TO MCODEO.
           MOVE WS-MSG(29)                  TO MMSGO.
       7230-GEST-ALTC-SUP-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
       7240-GEST-AUTRE-SUP-DEB.
           MOVE EIBTASKN                    TO MTASKO.
       7240-GEST-AUTRE-SUP-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
       7250-MOVE-MSG-DEJA-SUP-DEB.
           MOVE 'ENREGISTREMENT DEJA SUPPRIME'
                                            TO MMSGO.
       7250-MOVE-MSG-DEJA-SUP-FIN.
           EXIT.
      *
      *---------------------------------------------------------------*
      *   8XXX-  : ORDRES DE MANIPULATION DES EDITIONS                *
