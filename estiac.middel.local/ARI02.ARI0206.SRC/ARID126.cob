      ...
      *---------------------------------------------------------------*
      *  METTRE A JOUR LA TABLE DES COMPTES CLIENT A PARTIR D'UN      *
      *  FICHIER MOUVEMENTS BANCAIRES (F-MVTS-E).                     *
      ...
      *---------------------------------------------------------------*
      * 31/03/2025    §  CREATION DU FICHIER                         *
      *               §                                              *
      *===============================================================*
      *
      *************************
      ...
      *
       01  WS-SQLCODE                       PIC S9(9) COMP.
           88  TCPTES-OK                    VALUE 0.
           88  EOT-TCPTES                   VALUE 100.
      *
      ...
      *
       01  WS-DATE-TCPTES.
           05  WS-SS-TMPT                   PIC X(2).
           05  WS-AA-TMPT                   PIC X(2).
           05  FILLER                       PIC X VALUE '-'.
           05  WS-MM-TMPT                   PIC X(2).
           05  FILLER                       PIC X VALUE '-'.
           05  WS-JJ-TMPT                   PIC X(2).
      *
      ...
      *
           ...
           EXEC SQL INCLUDE TCPTES END-EXEC.
      *
      *                  ==============================               *
      *=================<    PROCEDURE      DIVISION   >==============*
      *                  ==============================               *
      ...
      *
           PERFORM 6050-SELECT-TCPTES-DEB
              THRU 6050-SELECT-TCPTES-FIN.
      *
      *---------------------------------------------------------------*
      * APPEL DU COMPOSANT SUIVANT                                    *
      *---------------------------------------------------------------*
      ...
      *
           PERFORM 3000-TRT-MVT-CPTE-EXIST-DEB
             ...
             UNTIL (WS-MVTS-CPTE NOT = WS-NUMCPTES) OR
                   EOF-MVTS.
      *
      *------------ SORTIE
      *
      ...
      *
              PERFORM 6110-UPDATE-TCPTES-DEB
                 THRU 6110-UPDATE-TCPTES-FIN
           ELSE
              PERFORM 7150-GST-OUT-CPTE-K-DEB
                 THRU 7150-GST-OUT-CPTE-K-FIN
      *
              PERFORM 6120-DELETE-TCPTES-DEB
                 THRU 6120-DELETE-TCPTES-FIN
           END-IF.
      *
      ...
      *
              PERFORM 6060-INSERT-TCPTES-DEB
                 THRU 6060-INSERT-TCPTES-FIN
           ELSE
      ...
      *
       6010-COMMIT-DEB.
           ...
           IF NOT TCPTES-OK
              DISPLAY 'PROBLEME COMMIT DE LA TABLE TCPTES'
      ...
      *
       6010-ROLLBACK-DEB.
           ...
           MOVE SQLCODE TO WS-SQLCODE.
           IF NOT TCPTES-OK
              DISPLAY 'PROBLEME ROLLBACK DE LA TABLE TCPTES'
              DISPLAY 'SQLCODE = ' SQLCODE
              PERFORM 9999-ERREUR-PROGRAMME-DEB
                 THRU 9999-ERREUR-PROGRAMME-FIN
           END-IF.
      ...
      *---------------------------------------------------------------*
      *
       6050-SELECT-TCPTES-DEB.
           EXEC SQL
              SELECT SLDCPTES
                INTO :WS-SLDCPTES
                FROM TCPTES
               WHERE NUMCPTES = :WS-NUMCPTES
           END-EXEC.
           MOVE SQLCODE                     TO WS-SQLCODE.
           IF NOT (TCPTES-OK OR EOT-TCPTES)
              MOVE SQLCODE                  TO WS-SQLCODE-DISP
              DISPLAY 'PROBLEME SELECT DE LA TABLE TCPTES'
              DISPLAY 'SQLCODE = ' WS-SQLCODE-DISP
              PERFORM 9999-ERREUR-PROGRAMME-DEB
                 THRU 9999-ERREUR-PROGRAMME-FIN
           END-IF.
       6050-SELECT-TCPTES-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
       6060-INSERT-TCPTES-DEB.
           EXEC SQL
              INSERT INTO TCPTES
                          (NUMCPTES,
                           DCRCPTES,
           ...
           IF NOT TCPTES-OK
              MOVE SQLCODE                  TO WS-SQLCODE-DISP
              DISPLAY 'PROBLEME INSERT DE LA TABLE TCPTES'
              DISPLAY 'SQLCODE = ' WS-SQLCODE-DISP
              PERFORM 9999-ERREUR-PROGRAMME-DEB
                 THRU 9999-ERREUR-PROGRAMME-FIN
           END-IF.
       6060-INSERT-TCPTES-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
      ...
      *---------------------------------------------------------------*
      *
       6110-UPDATE-TCPTES-DEB.
           EXEC SQL
              UPDATE TCPTES
                 SET SLDCPTES = :WS-SLDCPTES,
                     DMJCPTES = CURRENT DATE
               WHERE NUMCPTES = :WS-NUMCPTES
           END-EXEC.
           MOVE SQLCODE TO WS-SQLCODE.
           IF NOT TCPTES-OK
              MOVE SQLCODE                  TO WS-SQLCODE-DISP
              DISPLAY 'PROBLEME UPDATE DE LA TABLE TCPTES'
              DISPLAY 'SQLCODE = ' WS-SQLCODE-DISP
              PERFORM 9999-ERREUR-PROGRAMME-DEB
                 THRU 9999-ERREUR-PROGRAMME-FIN
           END-IF.
       6110-UPDATE-TCPTES-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
       6120-DELETE-TCPTES-DEB.
           EXEC SQL
              DELETE FROM TCPTES
               WHERE NUMCPTES = :WS-NUMCPTES
           END-EXEC.
           ...
           IF NOT (TCPTES-OK OR EOT-TCPTES)
              MOVE SQLCODE                  TO WS-SQLCODE-DISP
              DISPLAY 'PROBLEME DELETE DE LA TABLE TCPTES'
              DISPLAY 'SQLCODE = ' WS-SQLCODE-DISP
              PERFORM 9999-ERREUR-PROGRAMME-DEB
                 THRU 9999-ERREUR-PROGRAMME-FIN
           END-IF.
       6120-DELETE-TCPTES-FIN.
           EXIT.
      *---------------------------------------------------------------*
      *
      ...
      *
       7010-GST-IN-TRT-CPTE-DEB.
           MOVE WS-MVTS-CPTE                TO WS-NUMCPTES.
           MOVE ZERO                        TO WS-DCRCPTES
                                               WS-SLDCPTES
                                               WS-DMJCPTES.
           MOVE 0                           TO WS-CDB
                                               WS-CCR
      ...
      *
       7020-GST-IN-CPTE-YES-DEB.
           ...
           MOVE WS-NUMCPTES                 TO WS-LETAT-NUMCPT-ED.
           MOVE WS-SLDCPTES                 TO WS-LETAT-SOLD-TMP.
      ...
      *
       7040-GST-ENT-ETATCLI-DEB.
           ...
           MOVE WS-SLDCPTES                 TO WS-LETAT-SOLD-ED.
      ...
      *
       7060-DATE-1ST-MVT-DEB.
           MOVE WS-MVTS-SS                  TO WS-SS-TMPT.
           MOVE WS-MVTS-AA                  TO WS-AA-TMPT.
           MOVE WS-MVTS-MM                  TO WS-MM-TMPT.
           MOVE WS-MVTS-JJ                  TO WS-JJ-TMPT.
           MOVE WS-DATE-TCPTES              TO WS-DCRCPTES.
      ...
      *
       7140-GST-OUT-CPTE-STD-DEB.
           COMPUTE WS-LETAT-SOLD-ED =
                   WS-SLDCPTES + WS-CCR - WS-CDB.
           MOVE WS-LETAT-SOLD-ED            TO WS-SLDCPTES.
      ...
      *
       9999-FIN-PROGRAMME-DEB.
           PERFORM 6010-COMMIT-DEB
              THRU 6010-COMMIT-FIN.
           DISPLAY '*============================================*'.
           DISPLAY '*     FIN NORMALE DU PROGRAMME ARID126       *'.
           DISPLAY '*============================================*'.
       9999-FIN-PROGRAMME-FIN.
           EXIT.
      *
       9999-ERREUR-PROGRAMME-DEB.
           PERFORM 6010-ROLLBACK-DEB
              THRU 6010-ROLLBACK-FIN.
           DISPLAY '*============================================*'.
           DISPLAY '*        UNE ANOMALIE A ETE DETECTEE         *'.
           DISPLAY '*     FIN ANORMALE DU PROGRAMME ARID126      *'.
           DISPLAY '*============================================*'.
